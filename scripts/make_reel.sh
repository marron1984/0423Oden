...
make_scene() {
  ...
  local FC="[0:v]split=2[a][b];[a]scale=${W}:${H}:force_original_aspect_ratio=increase,crop=${W}:${H},gblur=sigma=32,eq=brightness=-0.32:saturation=0.85[bg];[b]scale=-1:1280:force_original_aspect_ratio=decrease[fg];[bg][fg]overlay=(W-w)/2:(H-h)/2-180[base];[base]drawbox=y=1480:x=0:w=${W}:h=${H}-1480:color=black@0.58:t=fill,drawbox=y=1540:x=100:w=4:h='if(lt(t\,0.6)\,t/0.6*260\,260)':color=${G}:t=fill,drawtext=fontfile=${FM}:text='${sub}':fontcolor=${G}:alpha='if(lt(t\,0.3)\,0\,if(lt(t\,0.8)\,(t-0.3)/0.5\,1))':fontsize=40:x=140:y=1555,drawtext=fontfile=${FK}:text='${m1}':fontcolor=white:alpha='if(lt(t\,0.6)\,0\,if(lt(t\,1.2)\,(t-0.6)/0.6\,1))':fontsize=80:x=140:y='1630+40*(1-min(1\,max(0\,(t-0.6)/0.6)))',drawtext=fontfile=${FK}:text='${m2}':fontcolor=white:alpha='if(lt(t\,0.85)\,0\,if(lt(t\,1.45)\,(t-0.85)/0.6\,1))':fontsize=80:x=140:y='1730+40*(1-min(1\,max(0\,(t-0.85)/0.6)))',fade=t=in:st=0:d=0.4,fade=t=out:st=${fo}:d=0.35"
  ffmpeg -y -loop 1 -t "$dur" -i "$src" -filter_complex "$FC" -r "$FPS" -pix_fmt yuv420p -c:v libx264 -preset medium -crf 20 "$out" 2>&1 | tail -1
}

make_final() {
  local dur="$1" out="$2"
  local FC="[0:v]format=yuv420p,drawbox=y=160:x=420:w='if(lt(t\,0.8)\,t/0.8*240\,240)':h=2:color=${G}:t=fill,drawbox=y=1758:x=420:w='if(lt(t\,0.8)\,t/0.8*240\,240)':h=2:color=${G}:t=fill,drawtext=fontfile=${FM}:text='Lunch Time':fontcolor=${G}:alpha='if(lt(t\,0.4)\,0\,if(lt(t\,1)\,(t-0.4)/0.6\,1))':fontsize=40:x=(w-text_w)/2:y=420,drawtext=fontfile=${FK}:text='おでん屋の、':fontcolor=white:alpha='if(lt(t\,0.7)\,0\,if(lt(t\,1.3)\,(t-0.7)/0.6\,1))':fontsize=92:x=(w-text_w)/2:y=500,drawtext=fontfile=${FK}:text='大満足ランチ。':fontcolor=white:alpha='if(lt(t\,1)\,0\,if(lt(t\,1.6)\,(t-1)/0.6\,1))':fontsize=102:x=(w-text_w)/2:y=620,drawtext=fontfile=${FB}:text='おでん×スタンド 三徳六味':fontcolor=white:alpha='if(lt(t\,1.4)\,0\,if(lt(t\,2)\,(t-1.4)/0.6\,1))':fontsize=48:x=(w-text_w)/2:y=960,drawtext=fontfile=${FM}:text='EST フードホール店':fontcolor=0xcccccc:alpha='if(lt(t\,1.55)\,0\,if(lt(t\,2.15)\,(t-1.55)/0.6\,1))':fontsize=42:x=(w-text_w)/2:y=1030,drawtext=fontfile=${FM}:text='大阪・梅田  /  11：00 ー 23：00':fontcolor=0xaaaaaa:alpha='if(lt(t\,1.7)\,0\,if(lt(t\,2.3)\,(t-1.7)/0.6\,1))':fontsize=38:x=(w-text_w)/2:y=1100,drawtext=fontfile=${FM}:text='RESERVATION':fontcolor=${G}:alpha='if(lt(t\,2.1)\,0\,if(lt(t\,2.7)\,(t-2.1)/0.6\,1))':fontsize=36:x=(w-text_w)/2:y=1340,drawtext=fontfile=${FK}:text='06-6743-4501':fontcolor=white:alpha='if(lt(t\,2.3)\,0\,if(lt(t\,2.9)\,(t-2.3)/0.6\,1))':fontsize=104:x=(w-text_w)/2:y=1420,fade=t=in:st=0:d=0.5"
...
echo "S1"; make_scene ${V}/s1.jpg 3 "Oden Stand Lunch" "おでん屋さんの、"     "本気のランチ。"      ${V}/c1.mp4
echo "S2"; make_scene ${V}/s2.jpg 3 "Best Price"       "とうめし定食"         "990円"              ${V}/c2.mp4
